...
export const TextToSpeech = ({
  ...
  // Update voice profile list whenever selectedLang changes
  useEffect(() => {
    const updateVoiceProfiles = () => {
      const languageVoices = speechService.getVoicesForLanguage(selectedLang);
      setAvailableVoices(languageVoices);
      if (languageVoices.length > 0) {
        setSelectedVoiceName(languageVoices[0].name);
      }
    };

    updateVoiceProfiles();
    if (typeof window !== "undefined" && window.speechSynthesis) {
      window.speechSynthesis.onvoiceschanged = updateVoiceProfiles;
    }
  ...
  const currentLangName = SUPPORTED_LANGUAGES.find(l => l.code === selectedLang)?.name || selectedLang;
  ...
  return (
    <div className={`bg-white rounded-2xl p-5 border border-slate-200/80 shadow-soft-sm ${className}`}>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h4 className="text-sm font-bold text-slate-900">
              {title || `Audio Narration: ${currentLangName}`}
            </h4>
            <p className="text-[11px] text-slate-500 font-medium">
              Native voice narration tailored for {currentLangName}
            </p>
          </div>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          {!isPlaying ? (
            ...
            >
              ...
              <span>{isPaused ? "Resume" : `Play Narration (${currentLangName})`}</span>
            </button>
          ) : (
            <button
              onClick={handlePause}
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-rose-50 text-slate-600 hover:text-rose-600 font-bold text-xs sm:text-sm transition-all disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
          >
            <Square className="w-3.5 h-3.5 fill-current" />
            <span>Stop</span>
          </button>
          ...
          <button
            onClick={handleReset}
            className="flex items-center gap-1.5 px-3 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-semibold transition-all cursor-pointer"
        ...
        {/* Speed presets & Language-Matched Voice Profile */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3 border-t border-slate-100">
          ...
          {/* Dynamic Voice Profile tailored to Selected Language */}
          {showVoiceSelector && (
            <div>
              <div className="flex items-center gap-1.5 text-xs font-bold text-slate-700 mb-2">
                <Languages className="w-3.5 h-3.5 text-brand-600" />
                <span>Voice Profile for {currentLangName}</span>
              </div>
              <select
                value={selectedVoiceName}
                onChange={(e) => setSelectedVoiceName(e.target.value)}
                className="w-full text-xs font-semibold bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-slate-800 focus:outline-none focus:ring-2 focus:ring-brand-500"
              >
